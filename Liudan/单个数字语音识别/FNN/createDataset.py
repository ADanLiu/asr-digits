...
def creatDataset():
    ...
    for i in dirs:
        ...
        test_random_list = random.sample(files, 500)
...
if __name__ == '__main__':
    (wavs, labels), (testwavs, testlabels), num_class = creatDataset()
    X_train = wavs.reshape(wavs.shape[0], -1).T  # 将维度为(a,b,c,d) 转换为(b ∗c ∗d, a)
    X_test = testwavs.reshape(testwavs.shape[0], -1).T
    print(X_train.shape, "   ", X_test.shape)

    Y_train = one_hot_matrix(labels, num_class)  # 将数据的label转换成one hot码
    Y_test = one_hot_matrix(testlabels, num_class)
